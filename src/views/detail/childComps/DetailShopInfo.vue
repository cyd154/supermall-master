    ...
    <div class="flex shop-other-info">
      ...
      <div class="shop-other-item shop-other-right">
        <div v-for="item in shop.score">
          {{item.name}}&ensp;
          <span class="score" :class="{ziti:item.isBetter}">{{item.score}}</span>
          <span class="score-bg" :class="{beijin:item.isBetter}">{{item.isBetter ? '高' : '低'}}</span>
        </div>
      </div>
    </div>
...
<style scoped>
  ...
  .shop-other-right .ziti {
    color: #d81e06;
  }
  .shop-other-right .score-bg {
    background-color: #006666;
    color: #fff;
    padding: 1px 4px;
    font-size: 12px;
  }
  .shop-other-right .beijin {
